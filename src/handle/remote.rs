...
impl Remote {
    ...
    pub fn make_temp_path(
        &self,
        who: &String,
        request: &Arc<TaskRequest>,
    ) -> Result<(PathBuf, PathBuf), Arc<TaskResponse>> {
    ...
    fn internal_run(
        ...
        assert!(
            ...
            "commands cannot be run in validate stage"
        );
            ...
            UseSudo::Yes => match self.template.add_sudo_details(request, &cmd) {
                ...
                Err(y) => {
                    return Err(self.response.is_failed(
                        request,
                        &format!("failure constructing sudo command: {}", y),
                    ));
                }
            },
        ...
        self.response
            ...
            .read()
            .expect("read visitor")
            .on_command_run(&self.response.get_context(), &Arc::clone(&self.host), &cmd);

        let result =
            self.connection
                .lock()
                .unwrap()
                .run_command(&self.response, request, &cmd_out, forward);
    ...
    pub fn get_os_type(&self) -> HostOSType {
        ...
        if os_type.is_none() {
            panic!(
                "failed to detect OS type for {}, bailing out",
                self.host.read().unwrap().name
            );
        }
    ...
    pub fn get_transfer_location(
        &self,
        request: &Arc<TaskRequest>,
    ) -> Result<(Option<PathBuf>, Option<PathBuf>), Arc<TaskResponse>> {
        let whoami = match self.get_whoami() {
            Ok(x) => x,
            Err(y) => {
                return Err(self
                    .response
                    .is_failed(request, &format!("cannot determine current user: {}", y)));
            }
        };
    ...
    fn conditionally_move_back(
        ...
        if temp_dir.is_some() {
            let move_to_correct_location = format!(
                "mv '{}' '{}'",
                temp_path.as_ref().unwrap().display(),
                desired_path
            );
    ...
    pub fn write_data<G>(
        ...
        data: &String,
        path: &String,
        mut before_complete: G,
    ) -> Result<(), Arc<TaskResponse>>
    where
        G: FnMut(&String) -> Result<(), Arc<TaskResponse>>,
    {
        ...
        self.response
            ...
            .expect("read visitor")
            .on_before_transfer(
                &self.response.get_context(),
                &Arc::clone(&self.host),
                &real_path,
            );
        let xfer_result = self.connection.lock().unwrap().write_data(
            &self.response,
            request,
            data,
            &real_path,
    ...
    pub fn fetch_file(
        ...
        let content =
            self.connection
                .lock()
                .unwrap()
                .fetch_file(&self.response, request, remote_src)?;
        self.run_state
            .fetched_files
            .lock()
            .unwrap()
            .insert(remote_src.clone(), content.clone());
        Ok(content)
    }
        ...
        G: FnMut(&String) -> Result<(), Arc<TaskResponse>>,
    {
        ...
        let xfer_result =
            self.connection
                .lock()
                .unwrap()
                .copy_file(&self.response, &request, src, &real_path)?;
        before_complete(&real_path.clone())?;
    ...
    pub fn get_is_directory(
        &self,
        request: &Arc<TaskRequest>,
        path: &String,
    ) -> Result<bool, Arc<TaskResponse>> {
        let get_cmd_result =
            crate::tasks::cmd_library::get_is_directory_command(self.get_os_type(), path);
        let cmd = self.unwrap_string_result(&request, &get_cmd_result)?;
    ...
    pub fn create_directory(
        ...
        let get_cmd_result =
            crate::tasks::cmd_library::get_create_directory_command(self.get_os_type(), path);
        let cmd = self.unwrap_string_result(&request, &get_cmd_result)?;
        return self.run(request, &cmd, CheckRc::Checked);
    }
    ...
    pub fn delete_file(
        ...
        let get_cmd_result =
            crate::tasks::cmd_library::get_delete_file_command(self.get_os_type(), path);
        let cmd = self.unwrap_string_result(&request, &get_cmd_result)?;
        return self.run(request, &cmd, CheckRc::Checked);
    }
    ...
    pub fn delete_directory(
        ...
        let get_cmd_result = crate::tasks::cmd_library::get_delete_directory_command(
            self.get_os_type(),
            path,
            recurse,
        );
        ...
        if path.eq("/") {
            return Err(self.response.is_failed(
                request,
                &String::from("accidental removal of / blocked by safeguard"),
            ));
        }
        return self.run(request, &cmd, CheckRc::Checked);
    }
    ...
    pub fn rename(
        ...
        let get_cmd_result =
            crate::tasks::cmd_library::get_rename_command(self.get_os_type(), src, dest, force);
        let cmd = self.unwrap_string_result(&request, &get_cmd_result)?;
        return self.run(request, &cmd, CheckRc::Checked);
    }
    ...
    pub fn file_exists(
        ...
        let get_cmd_result =
            crate::tasks::cmd_library::get_file_exists_command(self.get_os_type(), path);
        let cmd = self.unwrap_string_result(&request, &get_cmd_result)?;
        let result = self.run(request, &cmd, CheckRc::Unchecked)?;
    ...
    pub fn get_ownership(
        ...
        request: &Arc<TaskRequest>,
        path: &String,
    ) -> Result<Option<(String, String)>, Arc<TaskResponse>> {
        let get_cmd_result =
            crate::tasks::cmd_library::get_ownership_command(self.get_os_type(), path);
        let cmd = self.unwrap_string_result(&request, &get_cmd_result)?;
        ...
        let owner = match split.nth(2) {
            ...
            None => {
                return Err(self.response.is_failed(
                    request,
                    &format!("unexpected output format from {}: {}", cmd, out),
                ));
            }
        };
    ...
    pub fn set_owner(
        ...
        let get_cmd_result = crate::tasks::cmd_library::set_owner_command(
            self.get_os_type(),
            remote_path,
            owner,
            recurse,
    ...
    pub fn set_group(
        ...
        let get_cmd_result = crate::tasks::cmd_library::set_group_command(
            self.get_os_type(),
            remote_path,
            group,
            recurse,
    ...
    pub fn set_mode(
        ...
        let get_cmd_result = crate::tasks::cmd_library::set_mode_command(
            self.get_os_type(),
            remote_path,
            mode,
            recurse,
    ...
    pub fn query_common_file_attributes(
        &self,
        request: &Arc<TaskRequest>,
        remote_path: &String,
        ...
        if attributes_in.is_some() {
            ...
            if owner_result.is_none() {
                return Err(self.response.is_failed(
                    request,
                    &String::from("file was deleted unexpectedly mid-operation"),
                ));
            }
            ...
            if attributes.mode.is_some()
                && !remote_mode
                    .as_ref()
                    .unwrap()
                    .eq(attributes.mode.as_ref().unwrap())
            {
                changes.push(Field::Mode);
            }
        }
    ...
    pub fn process_common_file_attributes(
        &self,
        request: &Arc<TaskRequest>,
        remote_path: &String,
        attributes_in: &Option<FileAttributesEvaluated>,
                ...
                Field::Owner => {
                    if attributes.owner.is_some() {
                        self.set_owner(
                            request,
                            remote_path,
                            &attributes.owner.as_ref().unwrap(),
                            recurse,
                        )?;
                    }
                }
                Field::Group => {
                    if attributes.group.is_some() {
                        self.set_group(
                            request,
                            remote_path,
                            &attributes.group.as_ref().unwrap(),
                            recurse,
                        )?;
                    }
                }
    ...
    pub fn process_all_common_file_attributes(
        &self,
        request: &Arc<TaskRequest>,
        remote_path: &String,
        attributes_in: &Option<FileAttributesEvaluated>,
        ...
        return self.process_common_file_attributes(
            request,
            remote_path,
            attributes_in,
            &all,
