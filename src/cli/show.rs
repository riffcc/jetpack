...
use crate::util::terminal::{captioned_display, two_column_table};
use std::sync::Arc;
use std::sync::RwLock;
...
pub fn show_inventory_group(
    ...
    let host_elements: Vec<(String, String)> = vec![
        (
            format!("All Ancestors ({})", descendant_hosts_count),
            descendant_hosts_string,
        ),
        (
