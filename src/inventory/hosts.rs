...
use crate::inventory::dependencies::{DependencyBuilder, DependencyReader, VirtualizationType};
...
impl Host {
    ...
    pub fn set_os_info(&mut self, uname_output: &String) -> Result<(), String> {
        if uname_output.starts_with("Linux") {
            ...
            return Err(format!(
                "OS Type could not be detected from uname -a: {}",
                uname_output
            ));
        }
        Ok(())
    ...
    pub fn get_ancestor_groups(&self, depth_limit: usize) -> HashMap<String, Arc<RwLock<Group>>> {
        ...
        for (k, v) in self.get_groups().into_iter() {
            ...
            for (k2, v2) in v
                .read()
                .expect("group read")
                .get_ancestor_groups(depth_limit)
                .into_iter()
            {
    ...
    pub fn get_blended_variables(&self) -> serde_yaml::Mapping {
        ...
        for (_k, v) in ancestors.iter() {
            let theirs: serde_yaml::Value =
                serde_yaml::Value::from(v.read().unwrap().get_variables());
...
mod tests {
    ...
    fn test_set_os_info_unknown() {
        ...
        assert!(
            result
                .unwrap_err()
                .contains("OS Type could not be detected")
        );
    }
    ...
    fn test_checksum_cache() {
        ...
        // Initially empty
        assert!(
            host.get_checksum_cache(1, &"/path/file".to_string())
                .is_none()
        );
        ...
        assert!(
            host.get_checksum_cache(2, &"/path/file".to_string())
                .is_none()
        );
        ...
        assert_eq!(
            host.get_checksum_cache(2, &"/path/file1".to_string()),
            Some("checksum1".to_string())
        );
        assert_eq!(
            host.get_checksum_cache(2, &"/path/file2".to_string()),
    ...
    fn test_package_preference() {
        ...
        assert!(matches!(
            host.package_preference,
            Some(PackagePreference::Dnf)
        ));

        host.package_preference = Some(PackagePreference::Yum);
        assert!(matches!(
            host.package_preference,
            Some(PackagePreference::Yum)
        ));
    }
