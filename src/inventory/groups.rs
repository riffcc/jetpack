        ...
        for (k, v) in self.parents.iter() {
            ...
            if depth_limit > 0 {
                for (k2, v2) in v
                    .read()
                    .expect("group read")
                    .get_ancestor_groups(depth_limit - 1)
                {
        ...
        for (k, v) in self.subgroups.iter() {
            ...
            if depth_limit > 0 {
                for (k2, v2) in v
                    .read()
                    .expect("group read")
                    .get_descendant_groups(depth_limit - 1)
                    .iter()
                {
                    results.insert(k2.clone(), Arc::clone(&v2));
                }
    ...
    pub fn get_blended_variables(&self) -> serde_yaml::Mapping {
        ...
        for (_k, v) in ancestors.iter() {
            let theirs: serde_yaml::Value =
                serde_yaml::Value::from(v.read().expect("group read").get_variables());
...
mod tests {
    ...
    fn test_get_ancestor_groups() {
        ...
        parent
            .write()
            .unwrap()
            .add_parent(&"grandparent".to_string(), grandparent.clone());
    ...
    fn test_get_descendant_groups() {
        ...
        child
            .write()
            .unwrap()
            .add_subgroup(&"grandchild".to_string(), grandchild.clone());
