    ...
    fn evaluate(
        ...
        Ok(EvaluatedTask {
            action: Arc::new(ProxmoxLxcAction {
                ...
                hostname: handle.template.string_option(
                    request,
                    tm,
                    &String::from("hostname"),
                    &self.hostname,
                )?,
                ostemplate: handle.template.string_option(
                    request,
                    tm,
                    &String::from("ostemplate"),
                    &self.ostemplate,
                )?,
                storage: handle
                    .template
                    .string_option(request, tm, &String::from("storage"), &self.storage)?
                ...
                cores: handle
                    .template
                    .integer_option(request, tm, &String::from("cores"), &self.cores, Some(1))?
                    .unwrap_or(1),
                rootfs_size: handle
                    .template
                    .string_option(request, tm, &String::from("rootfs_size"), &self.rootfs_size)?
                    .unwrap_or_else(|| String::from("8G")),
                net0: handle.template.string_option(
                    request,
                    tm,
                    &String::from("net0"),
                    &self.net0,
                )?,
                password: handle.template.string_option(
                    request,
                    tm,
                    &String::from("password"),
                    &self.password,
                )?,
                unprivileged: handle.template.boolean_option_default_true(
                    request,
                    tm,
                    &String::from("unprivileged"),
                    &self.unprivileged,
                )?,
                start_on_create: handle.template.boolean_option_default_false(
                    request,
                    tm,
                    &String::from("start_on_create"),
                    &self.start_on_create,
                )?,
                state: handle
                    .template
                    .string_option(request, tm, &String::from("state"), &self.state)?
...
impl ProxmoxLxcAction {
        ...
        rt.block_on(async {
            ...
            let response = client
                ...
                .map_err(|e| {
                    handle
                        .response
                        .is_failed(request, &format!("Failed to query Proxmox API: {}", e))
                })?;
            ...
            let api_response: ProxmoxApiResponse<Vec<LxcListItem>> =
                response.json().await.map_err(|e| {
                    handle
                        .response
                        .is_failed(request, &format!("Failed to parse Proxmox response: {}", e))
                })?;
    ...
    fn create_container(
        ...
        rt.block_on(async {
            ...
            params.insert(
                "rootfs".to_string(),
                format!("{}:{}", self.storage, self.rootfs_size),
            );
            params.insert(
                "unprivileged".to_string(),
                if self.unprivileged { "1" } else { "0" }.to_string(),
            );
            params.insert(
                "start".to_string(),
                if self.start_on_create { "1" } else { "0" }.to_string(),
            );
            ...
            let response = client
                ...
                .map_err(|e| {
                    handle
                        .response
                        .is_failed(request, &format!("Failed to create container: {}", e))
                })?;
        ...
        rt.block_on(async {
            ...
            let response = client
                ...
                .map_err(|e| {
                    handle
                        .response
                        .is_failed(request, &format!("Failed to delete container: {}", e))
                })?;
        ...
        rt.block_on(async {
            ...
            let response = client
                ...
                .map_err(|e| {
                    handle
                        .response
                        .is_failed(request, &format!("Failed to stop container: {}", e))
                })?;
