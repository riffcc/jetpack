...
use crate::modules::proxmox::common::{
    GuestListItem, ProxmoxApiConfig, ProxmoxApiResponse, ProxmoxTaskStatus,
};
...
impl IsTask for ProxmoxMigrateTask {
    ...
    fn evaluate(
        ...
        let guest_type_str =
            handle
                .template
                .string(request, tm, &String::from("guest_type"), &self.guest_type)?;
        let guest_type = GuestType::from_str(&guest_type_str).ok_or_else(|| {
            handle.response.is_failed(
                request,
                &format!(
                    "Invalid guest_type '{}'. Must be 'vm' or 'lxc'",
                    guest_type_str
                ),
            )
        })?;
        ...
        Ok(EvaluatedTask {
            action: Arc::new(ProxmoxMigrateAction {
                ...
                source_node: handle.template.string(
                    request,
                    tm,
                    &String::from("source_node"),
                    &self.source_node,
                )?,
                target_node: handle.template.string(
                    request,
                    tm,
                ...
                vmid: handle
                    .template
                    .integer(request, tm, &String::from("vmid"), &self.vmid)?,
                guest_type,
                online: handle.template.boolean_option_default_true(
                    request,
                    tm,
                    &String::from("online"),
                    &self.online,
                )?,
                restart: handle.template.boolean_option_default_true(
                    request,
                    tm,
                    &String::from("restart"),
                    &self.restart,
...
impl ProxmoxMigrateAction {
    ...
    fn find_guest_node(
        ...
        rt.block_on(async {
            ...
            if self
                .guest_exists_on_node(&client, &self.source_node, handle, request)
                .await?
            {
            ...
            // Check target node
            if self
                .guest_exists_on_node(&client, &self.target_node, handle, request)
                .await?
            {
    ...
    async fn guest_exists_on_node(
        ...
        let url =
            self.api_config
                .get_api_url(&format!("/nodes/{}/{}", node, self.guest_type.api_path()));
        ...
        let response = client
            ...
            .map_err(|e| {
                handle
                    .response
                    .is_failed(request, &format!("Failed to query {}: {}", node, e))
            })?;
        ...
        let api_response: ProxmoxApiResponse<Vec<GuestListItem>> =
            response.json().await.map_err(|e| {
                handle
                    .response
                    .is_failed(request, &format!("Failed to parse response: {}", e))
            })?;
        ...
        rt.block_on(async {
                ...
                .await?
            {
                return Err(handle.response.is_failed(
                    request,
                    &format!(
                        "Migration completed but guest {} not found on target node {}",
                        self.vmid, self.target_node
                    ),
                ));
            }
            ...
            handle.debug(
                request,
                &format!(
                    "Guest {} successfully migrated to {}",
                    self.vmid, self.target_node
                ),
            );
    ...
    async fn start_migration(
        ...
        match self.guest_type {
            GuestType::Vm => {
                params.insert(
                    "online".to_string(),
                    if self.online { "1" } else { "0" }.to_string(),
                );
            }
            GuestType::Lxc => {
                params.insert(
                    "restart".to_string(),
                    if self.restart { "1" } else { "0" }.to_string(),
                );
            }
        }
        ...
        let response = client
            ...
            .map_err(|e| {
                handle
                    .response
                    .is_failed(request, &format!("Failed to start migration: {}", e))
            })?;
        ...
        if !response.status().is_success() {
            ...
            return Err(handle.response.is_failed(
                request,
                &format!("Migration API returned status {}: {}", status, text),
            ));
        }

        let migrate_response: MigrateResponse = response.json().await.map_err(|e| {
            handle.response.is_failed(
                request,
        ...
        migrate_response.data.ok_or_else(|| {
            handle.response.is_failed(
                request,
                &String::from("Migration started but no task UPID returned"),
            )
        })
    }
        ...
        loop {
            ...
            let response = client
                ...
                .map_err(|e| {
                    handle
                        .response
                        .is_failed(request, &format!("Failed to check task status: {}", e))
                })?;
            ...
            if !response.status().is_success() {
                ...
                return Err(handle.response.is_failed(
                    request,
                    &format!("Task status API returned {}: {}", status, text),
                ));
            }

            let api_response: ProxmoxApiResponse<ProxmoxTaskStatus> =
                response.json().await.map_err(|e| {
                    handle
                        .response
                        .is_failed(request, &format!("Failed to parse task status: {}", e))
                })?;
            ...
            if let Some(task_status) = api_response.data {
                match task_status.status.as_str() {
                    ...
                    other => {
                        return Err(handle
                            .response
                            .is_failed(request, &format!("Unexpected task status: {}", other)));
                    }
                }
            } else {
                return Err(handle
                    .response
                    .is_failed(request, &String::from("No task status data returned")));
            }
        }
    }
}
