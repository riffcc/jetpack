    ...
    fn evaluate(
        ...
        Ok(EvaluatedTask {
            action: Arc::new(UserAction {
                ...
                uid: handle.template.integer_option(
                    request,
                    tm,
                    &String::from("uid"),
                    &self.uid,
                ...
                gid: handle
                    .template
                    .string_option(request, tm, &String::from("gid"), &self.gid)?,
                groups: {
                    match &self.groups {
                        Some(groups) => {
                            let mut templated_groups: HashSet<String> = HashSet::new();
                            for group in groups {
                                templated_groups.insert(handle.template.string_no_spaces(
                                    request,
                                    tm,
                                    &String::from("groups"),
                                    group,
                ...
                gecos: handle.template.string_option(
                    request,
                    tm,
                    &String::from("gecos"),
                    &self.gecos,
                )?,
                shell: handle.template.string_option(
                    request,
                    tm,
                    &String::from("shell"),
                    &self.shell,
            ...
            TaskRequestType::Query => {
                ...
                if os_type != HostOSType::Linux {
                    return Err(handle.response.is_failed(
                        request,
                        &String::from("this user module only supports Linux"),
                    ));
                }
                ...
                match (actual.exists, self.remove) {
                    ...
                    (true, false) => {
                        ...
                        if UserAction::u64_wants_change(&self.uid, &actual.uid) {
                            changes.push(Field::Uid);
                        }
                        if UserAction::string_wants_change(&self.gid, &actual.gid) {
                            changes.push(Field::Gid);
                        }
                        if UserAction::string_wants_change(&self.gecos, &actual.gecos) {
                            changes.push(Field::Gecos);
                        }
                        if UserAction::string_wants_change(&self.shell, &actual.shell) {
                            changes.push(Field::Shell);
                        }
                        if self.groups_wants_change(&actual) {
                            changes.push(Field::Groups);
                        }
...
impl UserAction {
    ...
    fn get_groups(
        ...
        handle: &Arc<TaskHandle>,
        request: &Arc<TaskRequest>,
    ) -> Result<HashSet<String>, Arc<TaskResponse>> {
    ...
    fn get_user_details(
        ...
        handle: &Arc<TaskHandle>,
        request: &Arc<TaskRequest>,
    ) -> Result<UserDetails, Arc<TaskResponse>> {
