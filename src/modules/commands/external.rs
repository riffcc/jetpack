                ...
                params: {
                    let params_data = match serde_json::to_string(&self.params) {
                        ...
                        Err(_y) => {
                            return Err(handle
                                .response
                                .is_failed(request, &String::from("unable to load JSON inputs")));
                        }
                    };
                    // note: this level of templating occurs at runtime and we would /like/ to do it earlier.
                    match handle.template.string_unsafe_for_shell(
                        request,
                        TemplateMode::Strict,
                        &String::from("params"),
                        &params_data,
                        ...
                        Err(y) => {
                            return Err(handle.response.is_failed(
                                request,
                                &format!("unable to template parameters: {:?}", y),
                            ));
                        }
                    }
                },
    ...
    fn dispatch(
        ...
        match request.request_type {
            ...
            TaskRequestType::Execute => {
                ...
                let module_run =
                    format!("{} < {}", module_str_path.clone(), param_str_path.clone());
                let task_result =
                    handle
                        .remote
                        .run_unsafe(request, &module_run, CheckRc::Checked)?;
                let (rc, out) = cmd_info(&task_result);

                handle
                    .remote
                    .delete_file(request, &param_str_path.clone())?;
                handle
                    .remote
                    .delete_file(request, &module_str_path.clone())?;
...
fn build_results_map(
    ...
    let data: serde_yaml::Value = match serde_yaml::from_str(out) {
        ...
        Err(_y) => {
            return Err(handle.response.is_failed(
                request,
                &format!("failed to parse external module response: {}", out),
            ));
        }
    };
